...
router.post("/", async (req, res) => {
    try {
        ...
        //Logic
        const { Name, GroupExercises, ExerciseIDs, SetIDs } = req.body;
        const newWorkout = await pool.query(
            `INSERT INTO workoutStructure (Name, GroupExercises, ExerciseIDs, SetIDs)
             VALUES ($1, $2, $3, $4) RETURNING *`,
            [Name, GroupExercises, ExerciseIDs, SetIDs]
        );
    ...
    try {
      ...
      const updatedWorkout = await pool.query(
        `UPDATE workoutStructure SET
           ...
           GroupExercises = COALESCE($2, GroupExercises),
           ExerciseIDs = COALESCE($3, ExerciseIDs),
           SetIDs = COALESCE($4, SetIDs)
         WHERE WorkoutID = $5
         RETURNING *`,
        [Name, GroupExercises, ExerciseIDs, SetIDs, id]
      );
